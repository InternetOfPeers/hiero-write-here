...
- Forward secrecy via ephemeral keys (each message uses unique key pair)
...
5. **Forward Secrecy (ECIES)**: Compromise of long-term keys doesn't reveal past messages
...
3. **Key Compromise**: Stolen private keys can decrypt messages
   - Mitigation: Secure key storage; ECIES provides forward secrecy (past messages remain secure)

### Best Practices

- Store private keys encrypted at rest
- Use ECIES when possible for forward secrecy (secp256k1 accounts only)
